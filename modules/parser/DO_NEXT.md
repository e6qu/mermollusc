...
- **Gantt family arc** — **done & activated** (parser #104, layout #105, activation: union +
  ...
  **structural task delete** (select a bar + Delete → `deleteGanttTask` removes its source line by the
  label span, robust for auto-id tasks; multi-delete applied bottom-up so spans stay valid). Gantt is
  now at full editing parity (relabel + delete) with the other families.

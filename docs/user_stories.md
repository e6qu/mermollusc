...
| SRC-03 | As a diagram author, I get actionable parse feedback. | Parse errors show status text with line/column, dim stale canvas, mark the editor span, hide selection-anchored editing chrome (the context bar) while stale, and clear when fixed. | Covered: `editor.spec.ts`, `stale-ux.spec.ts`, malformed-family specs, parser `ParseError.positions` tests. |
| SRC-04 | As a diagram author, empty or truncated input must not crash the editor. | Truncated input surfaces a located parse error; an emptied source shows a friendly recovery empty state ("nothing to render — type a diagram or load an example"), not lexer jargon or a ghost render; no uncaught exception. | Covered: `editor.spec.ts`, `stale-ux.spec.ts`, parser robust tests. |
...
| FAM-11 | gitGraph authors can render commit history, commit ids, and merge/commit tags, and relabel explicit commit ids. | Branch/checkout/merge commands lay out into lanes; explicit `id:` and `tag:` annotations render as captions on the classic rail; malformed refs fail loudly. | Covered: `gitgraph.spec.ts`, parser/layout gitGraph tests (classic id/tag captions). |
| FAM-12 | Timeline authors can render title, sections, periods, events, continuations, and relabel text. | The `title` line renders centered above the chart; sections band periods; invalid continuation before a period is surfaced. | Covered: `timeline.spec.ts`, parser/layout timeline tests (incl. title). |
...
| FAM-14 | Pie authors can render pies and donuts with a title, labels, legends, and `showData`. | The `title` line renders centered above the chart; non-positive slices fail loudly; donut wedges render in canvas and SVG with an empty hole (canvas + SVG agree); legend swatches stay full discs. | Covered: `pie.spec.ts`, renderer wedge tests (donut hole anticlockwise sweep), layout title test, shot `32-pie-donut`. |
| FAM-15 | Gantt authors can render a title, dated tasks, milestones, after-chains, exclusions, tick intervals, and edit/delete tasks. | The `title` renders centered above the chart; working days stretch bars across exclusions; a milestone renders as a compact diamond with its label placed beside it; task relabel/delete patches source safely. | Covered: `gantt.spec.ts`, `gantt-edit.spec.ts`, parser/layout Gantt tests (title + milestone geometry). |
| FAM-16 | DOT users can import/export Graphviz-compatible graph text. | DOT import renders as a flowchart; DOT export downloads a graph from the current scene. DOT import is read-only on the canvas (no `parseDotWithSource` span map yet), so add/connect/relabel/reshape/place are disabled with a "DOT import is read-only — edit the source text" affordance rather than failing after a click. | Covered: `dot.spec.ts`, `dot-export.spec.ts`, `dot-roundtrip.test.ts`, `connect-families.spec.ts` (DOT read-only gating). |
...
| CAN-01a | As a visual editor, selected edges are visibly actionable. | Edge selection draws a route highlight and label-anchor marker; task guidance names edge relabel/delete actions. | Partial: the task-guidance + selection wiring is asserted in `audit-fixes.spec.ts`; the route-highlight/label-anchor pixels are reviewed via shot `02-edge-selected` (no automated draw-call assertion yet — see DO_NEXT). |
...
| CAN-08 | As a visual editor, I can Relax and Regenerate layout intentionally. | Relax seeds ELK with current positions; Regenerate preserves pinned overrides and clears unpinned override state; both are undoable. | Partial: `toolbar.spec.ts` asserts Regenerate **preserves** a pinned override and Relax/Regenerate run cleanly; the **clears-unpinned** clause is only reachable via an imported overlay (drag/resize always pin), so it's covered by the `pinnedOverrides` filter at `main.ts` — an end-to-end assertion is a DO_NEXT item. |
...
| COL-06 | As an enterprise operator, auth and persistence seams are explicit. | Browser Auth0 PKCE supplies first-frame tokens; the relay verifies them; a static membership file can authorize rooms server-side; file store persists room snapshots; production store remains a documented next step. | Covered: collab auth/store/membership/relay tests and app auth integration tests. |
...
| COL-08 | As a collaborator, joining a room never flashes the wrong diagram and never strands me on a blank editor. | Collab boot shows a "joining the shared room" state and paints only when the first `Y.Text` sync lands (then auto-fits); if the relay is permanently unreachable, the seed gate releases so the initial source still renders. | Covered: `collab-flag.spec.ts`, collab session/transport tests; join status wiring in `main.ts`. |
...
- Extend the backend-free Pages e2e if future Pages routing, build flags, or embedded browser-store
  wiring changes.

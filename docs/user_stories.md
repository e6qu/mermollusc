...
| COL-06 | As an enterprise operator, auth and persistence seams are explicit. | Auth0 verifier validates first-frame tokens; file store persists room snapshots; production store remains a documented next step. | Covered: collab auth/store/relay tests. |

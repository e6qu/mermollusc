...
| COL-06 | As an enterprise operator, auth and persistence seams are explicit. | Browser Auth0 PKCE supplies first-frame tokens; the relay verifies them; file store persists room snapshots; production store remains a documented next step. | Covered: collab auth/store/relay tests and app auth integration tests. |

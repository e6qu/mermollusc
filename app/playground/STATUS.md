...
- **Family-aware controls:** an **Examples** menu drops a known-good starter for each of the thirteen
...
- `main.ts`: source editor (CodeMirror) ↔ canvas.
  - edit text → re-render via `parseDiagram` + `layoutDiagram` (all thirteen families; gitGraph is
    render + inline commit-id relabel, timeline render + inline period/event relabel, mindmap render +
    inline node relabel — no structural edit for those three);
